exports.run = (client, msg, args) => {
    ...
    if ("408634582087368715" != msg.channel.id && "599618251793891368" != msg.channel.id) {
		msg.delete();
		return msg.channel.send(`:x: <@${msg.author.id}> команды надо писать в <#408634582087368715>`).then(msg => msg.delete(10000));
	}
    ...
    client.req({url: `${client.url_api_alisa}?key=${client.KAA}&a=images&select=${args[1]}`, json: true}, (error, response, body) => {
        ...
        if (args[2]) {
            ...
            for (var i = 2; i < args.length; i++) {
                // let listuserjson = msg.guild.member(args[i].replace('<@', '').replace('>', ''));
                // if (!listuserjson) msg.guild.fetchMember(listuserjson).id;
        ...
        random_images_get = Math.floor(Math.random() * body.files.length);
        let embed = new client.discord.RichEmbed().setDescription(text_message_file[args[1]]).setColor('#3498db').setImage(`https://sovietsoft.ru/alisabot/images/${args[1]}/${response.body.files[random_images_get].file}`)
